...
class OrderLogic
{
    ...
    public static function create_transaction($order, $received_by=false, $status = null)
    {
        ...
        $comission_on_delivery = $value*($order->original_delivery_charge/100);
        ...
        try{
            OrderTransaction::insert([
                ...
                'delivery_charge'=>$order->delivery_charge-$comission_on_delivery,//minus here
            ...
            $adminWallet->total_commission_earning = $adminWallet->total_commission_earning+$comission_amount+$comission_on_delivery;//add here
            ...
            try
            {
                ...
                else if($received_by=='restaurant' && $order->payment_method == 'cash_on_delivery')
                {
                    ...
                    if($order->restaurant->self_delivery_system)
                    {
                        $vendorWallet->collected_cash = $vendorWallet->collected_cash + ($order->delivery_charge <= 0 ? $order->delivery_charge : $order->delivery_charge - $comission_on_delivery);
